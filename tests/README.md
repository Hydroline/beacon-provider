...
- 顺序调用 `mtr:get_routefinder_snapshot`、`mtr:get_routefinder_data`、`mtr:get_routefinder_state`、`mtr:get_routefinder_edges`、`mtr:get_routefinder_cache`、`mtr:get_connection_profile`、`mtr:get_platform_position_map`、`mtr:get_rail_curve_segments`、`mtr:get_routefinder_version`，每次都把响应记录在 `mtr_<action>_<dimension>.json` 供调试参考；
