...
| `mtr:get_route_trains` | 返回指定维度/线路上正在运行的列车列表及对应的轨道 ID（`railId`）。 | 可选：`dimension`、`routeId`（默认 `0` 表示所有线路）。 | `timestamp`、可选的 `dimension`/`routeId`、`trains`（含 `trainUuid`、`trainId`、`baseTrainType`、`routeId`、`railId`、`currentStationId`、`nextStationId`、`progress`、`segmentCategory`、`delayMillis`、`passengers`（含 `uuid`, `name`）等） |
